...
## Data acquisition
The [VanouvR package](https://mountainmath.github.io/VancouvR/) makes it easy to access this data in R and [one of the package vignettes](https://mountainmath.github.io/VancouvR/articles/Demo.html) has code that does pretty much what we need.
...
Since this is ratio data we chose a logarithmic scale on the y-axis. This shows that between 2005 and 2021 (so using assessment years 2006 and 2022) real land values increased by `r scales::percent(filter(plot_data,Date=="2021-07-01",Component=="Land")$real_ratio)` and building values by `r scales::percent(filter(plot_data,Date=="2021-07-01",Component=="Building")$real_ratio)`  . Maybe the article was using nominal value increases, in nominal terms land increased by `r scales::percent(filter(plot_data,Date=="2021-07-01",Component=="Land")$ratio)` and building values by `r scales::percent(filter(plot_data,Date=="2021-07-01",Component=="Building")$ratio)`. 
